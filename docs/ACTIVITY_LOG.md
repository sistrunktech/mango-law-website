...
Date/time: 2026-03-30 17:35 EDT
Category: SEO / Discovery Reinforcement + March Reporting
Summary: Pulled a fresh March 30 Search Console analytics + URL Inspection snapshot, confirmed the sampled recovery set is still `7 unknown / 1 indexed`, strengthened discovery links from indexed surfaces into the stalled OVI/resource/blog cluster, tightened the main OVI pillar to separate broad strategy from narrower issue pages, and refreshed the March report-prep notes with the current bottleneck and metrics.
Files: docs/client-updates/2026-03-prep.md; docs/ACTIVITY_LOG.md; src/components/BlogSection.tsx; src/views/HomePage.tsx; src/views/BlogPage.tsx; src/components/BlogSidebar.tsx; src/views/OviDuiPage.tsx; src/data/blogPosts.ts; scripts/gsc-submit-sitemap.mjs; package.json
Notes: Current 7-day Search Console window is `12 clicks / 280 impressions / avg position 17.32`; the priority URLs still undiscovered in both properties are `/ovi-dui-defense-delaware-oh`, `/resources/dui-checkpoints`, `/ovi-test-refusal-lawyer-ohio`, `/als-license-suspension-ohio`, `/motion-to-suppress-ovi-ohio`, `/blog/ohio-ovi-driving-privileges-als`, and `/blog/drug-possession-charge-ohio-what-to-do-next`. Manual `Request indexing` remains blocked by browser-tool instability in this runtime, so the clean release path will rely on on-site discovery improvements plus fresh sitemap submission.

Date/time: 2026-04-01 11:45 EDT
Category: DUI Checkpoints / Precision Cleanup + Map Integrity
Summary: Implemented an exact-vs-approximate checkpoint location model across the public map, checkpoint cards, admin editors, and scraper pipeline; added city/county-centroid fallback for vague public notices; and ran a live checkpoint backfill to eliminate missing coordinates while clearly marking approximate pins instead of implying false street-level precision.
Files: src/data/checkpoints.ts; src/components/CheckpointMap.tsx; src/components/CheckpointCard.tsx; src/components/GeocodingPreview.tsx; src/views/DUICheckpointsPage.tsx; src/views/CheckpointAdminPage.tsx; src/components/admin/CheckpointManager.tsx; supabase/functions/checkpoint-scraper/geocoding.ts; supabase/functions/checkpoint-scraper/index.ts; scripts/backfill-geocoding.ts
Notes: After the April 1 live backfill, the `dui_checkpoints` table has `295` rows with `0` missing coordinates. The current precision mix is `34 exact_high`, `14 exact_medium`, `37 exact_low`, `14 city_centroid`, `2 county_centroid`, plus older exact rows still carrying legacy `high/medium/low` confidence values from prior runs. The map can now render all recent-history records without hiding rows that lack coordinates, and approximate pins are explicitly labeled as city/county area markers when the source did not publish a street address.

Date/time: 2026-04-01 12:46 EDT
Category: DUI Checkpoints / Map UX Debug
...
Date/time: 2026-04-05 16:45 EDT
Category: DUI Checkpoints / Automation Freshness
...
Date/time: 2026-08-05 10:45 EDT
Category: Production Release / Checkpoint Relevance + Release Safety
Summary: Promoted the completed checkpoint curation release from exact `origin/main`, verified the deployed scraper and schedule, hardened the public checkpoint views, removed airport/TSA/cancelled/aggregator noise, and completed candidate plus production map, lead, and indexing acceptance checks.
Files: supabase/functions/checkpoint-scraper/*; supabase/migrations/20260801023000_harden_public_checkpoint_read_views.sql; src/app/(site)/resources/dui-checkpoints/page.tsx; src/lib/checkpointPublicRelevance.ts; src/views/DUICheckpointsPage.tsx; docs/technical/MANGO_LAW_AUGUST_EXECUTION_BOARD_2026-08-05.md
Notes: Source `8186c636a60989c9253af9328672e0c510974017` is live as Worker version `65b1a980-e47b-499e-b105-4ebfd61301a5` through deployment `7f71f6e9-2a87-4749-bcbd-cf574bce30f7`; rollback is `1ee23e0a-bf32-47ca-9fd3-42ef72882df8`. Production map smoke rendered a visible Mapbox canvas with successful style and both public-view responses. Public views expose 57 checkpoints and 60 announcements; the UI shows 56 recent-history map items and a feed-derived August 5 12:17 p.m. UTC source check, with Brook Park as the latest direct-source announcement. Final zero-write lead QA intercepted both synthetic submissions, blocked 30 analytics calls, and left zero synthetic rows in all three lead tables. The first immediate post-promotion lead smoke stopped before form availability; after a 204/0.41s backend CORS check and visible-form confirmation, one deliberate non-writing repeat passed. No second real canary was sent because lead source and Edge functions were unchanged after the already-verified release canary.
