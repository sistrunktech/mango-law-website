...
import { blogIssuePathGroups } from '../data/blogDiscovery';
            ...
            <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
              <div>
                ...
                <h2 className="mt-2 text-2xl font-bold leading-tight text-brand-black">
                  Start with the article cluster that matches the pressure point.
                </h2>
              </div>
              <p className="max-w-xl text-sm leading-relaxed text-brand-black/60">
                These paths connect the newest guides to the main defense pages for OVI, criminal cases, protection-order
                issues, domestic allegations, and drug charges.
              </p>
            </div>

            <div className="mt-6 grid gap-4 lg:grid-cols-2 xl:grid-cols-4">
              {blogIssuePathGroups.map((group) => (
                <div
                  key={group.label}
                  className="rounded-[1.25rem] border border-brand-black/10 bg-brand-offWhite/45 p-5"
                >
                  <h3 className="text-base font-bold text-brand-black">{group.label}</h3>
                  <p className="mt-2 text-sm leading-relaxed text-brand-black/65">{group.summary}</p>
                  <div className="mt-5 space-y-3">
                    {group.links.map((item) => (
                      <Link
                        key={item.href}
                        href={item.href}
                        className="group block rounded-xl bg-white px-4 py-3 shadow-sm transition-all hover:-translate-y-0.5 hover:shadow-soft"
                      >
                        <span className="text-sm font-semibold text-brand-black transition-colors group-hover:text-brand-mangoText">
                          {item.title}
                        </span>
                        <span className="mt-1 block text-xs leading-relaxed text-brand-black/60">
                          {item.description}
                        </span>
                      </Link>
                    ))}
