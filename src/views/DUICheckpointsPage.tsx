...
export default function DUICheckpointsPage({
  ...
  const latestAnnouncementDateKind =
    latestAnnouncement && isFuturePublicDate(latestAnnouncementDate, now) ? 'Upcoming' : 'Published';
  const latestConfirmedDateKind =
    latestConfirmedCheckpoint && isFuturePublicDate(latestConfirmedCheckpoint.start_date, now)
      ? 'Upcoming'
      : 'Recent';
            ...
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1">
                ...
                <p className="mb-3 text-sm text-brand-black/80">
                  This map shows <strong>only publicly announced OVI checkpoints</strong> in Ohio from verified sources. Data is compiled from official law enforcement announcements, news outlets, and verified public sources, with non-Ohio checkpoint articles filtered out. Not all checkpoints are announced in advance, and this map does not predict or speculate about unannounced locations.
                </p>
              ...
              <div className="mt-4 grid gap-3 sm:grid-cols-2">
                <div className="rounded-xl border border-brand-black/10 bg-brand-offWhite px-4 py-3">
                  ...
                  <div className="mt-1 text-xs text-brand-black/60">
                    {latestAnnouncement
                      ? `${latestAnnouncementDateKind}: ${formatDisplayDate(latestAnnouncementDate) || 'Date unavailable'}${latestAnnouncement.source_name ? ` • ${latestAnnouncement.source_name}` : ''}`
                ...
                <div className="rounded-xl border border-brand-black/10 bg-brand-offWhite px-4 py-3">
                  ...
                  <div className="mt-1 text-xs text-brand-black/60">
                    {latestConfirmedCheckpoint
                      ? `${latestConfirmedDateKind}: ${formatDisplayDate(latestConfirmedCheckpoint.start_date) || 'Date unavailable'}${latestConfirmedCheckpoint.location_county ? ` • ${latestConfirmedCheckpoint.location_county} County` : ''}`
                  ...
                  <div key={item.key} className="rounded-xl border border-brand-black/10 bg-brand-offWhite px-4 py-3">
                    <div className="flex flex-wrap items-start justify-between gap-2">
                      ...
                      <span className="rounded-full bg-brand-black/5 px-2.5 py-1 text-xs font-semibold text-brand-black/70">
                        {isFuturePublicDate(item.publishedDate, now) ? 'Upcoming: ' : ''}
